...
class GolfDB(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        cap_pose = np.load(osp.join(self.pose_dir, '{}.npy'.format(a['id'])))
        ...
        if self.train:
            ...
            while len(images) < self.seq_length:
                ...
                if ret:
                    ...
                    # add landmarks
                    landmarks.append(list(cap_pose[pos]))
        ...
        sample = {'images':np.asarray(images), 'landmarks':np.asarray(landmarks), 'labels':np.asarray(labels)}
        if self.transform:
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        images, landmarks, labels = sample['images'], sample['landmarks'], sample['labels']
...
class Normalize(object):
    ...
    def __call__(self, sample):
        ...
        return {'images': images, 'landmarks': landmarks, 'labels': labels}
...
if __name__ == '__main__':
    ...
    dataset = GolfDB(data_file='/content/drive/MyDrive/Github/joeklein-gaalevy-wemarti-fp/data/train_split_1.pkl',
                     vid_dir='/content/drive/MyDrive/Github/joeklein-gaalevy-wemarti-fp/data/videos_160/',
                     pose_dir='/content/drive/MyDrive/Github/joeklein-gaalevy-wemarti-fp/data/videos_160_human_pose/',
                     seq_length=64,
